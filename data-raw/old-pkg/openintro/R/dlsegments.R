...
function(x1=c(3,7), x2=c(5,9), l=c("o","o"), r=c("c", "c"), type=c("n", "u", "i"), COL=2, lwd=2.224, ylim=c(-0.35,2), mar=rep(0,4), hideOrig=FALSE){
  ...
  if (!hideOrig) {
    if(code1 > 0){
      arrows(x1[1], 1.1082, x1[2], 1.1082,
             length=0.07, code=code1, lwd=0.592)
    } else {
      lines(x1, rep(1.1082, 2), lwd=0.592)
    }
    if(x1[1] > xlim[1]){
      points(x1[1], 1.1082, pch=19, col="#FFFFFF")
      points(x1[1], 1.1082, pch=ifelse(l[1] == "c", 19, 1))
    }
    if(x1[2] < xlim[2]){
      points(x1[2], 1.1082, pch=19, col="#FFFFFF")
      points(x1[2], 1.1082, pch=ifelse(r[1] == "c", 19, 1))
    }
  }
  ...
  if (!hideOrig) {
    if(code2 > 0){
      arrows(x2[1], 1.729, x2[2], 1.729,
             length=0.07, code=code2, lwd=0.592)
    } else {
      lines(x2, rep(1.729, 2), lwd=0.592)
    }
    if(x2[1] > xlim[1]){
      points(x2[1], 1.729, pch=19, col="#FFFFFF")
      points(x2[1], 1.729, pch=ifelse(l[2] == "c", 19, 1))
    }
    if(x2[2] < xlim[2]){
      points(x2[2], 1.729, pch=19, col="#FFFFFF")
      points(x2[2], 1.729, pch=ifelse(r[2] == "c", 19, 1))
    }
  }
  
  #___ Build Axis ___#
  arrows(xlim[1], 0.55012, xlim[2], 0.55012,
  ...
  if (TRUE) { # (ticks) {
    if (diff(xlim) < 11) {
      ...
      if (abs(tail(xs, 1) - xlim[2]) < diff(xlim)/20) {
        xs <- head(xs, -1)
      }
    } else {
      xs <- union(x1, x2)
  ...
  if (tolower(substr(type, 1, 1)) == "i") {
    ...
    if (left < right) {
      ...
      lines(c(left,right), rep(0.55012, 2),
            lwd=lwd, col=COL[1])
      points(c(left, right), rep(0.55012, 2),
             pch=19, col="#FFFFFF")
      points(c(left, right), rep(0.55012, 2),
             pch=pch, col=COL[1])
      ...
      if (pch1 == "c" && pch[2] == "c") {
        ...
        points(X, 0.55012, pch=19, col=COL[1])
      }
    }
  }
  ...
  if (tolower(substr(type, 1, 1)) == "u") {
    if(code1 > 0){
      arrows(x1[1], 0.55012, x1[2], 0.55012,
    ...
    if(code2 > 0){
      arrows(x2[1], 0.55012, x2[2], 0.55012,
    ...
    #___ Add Points ___#
    pch <- ifelse(pch == "o", 1, 19)
    points(X, rep(0.55012, length(X)), pch=19, col="#FFFFFF")
    points(X, rep(0.55012, length(X)), pch=pch, col=COL[1])
